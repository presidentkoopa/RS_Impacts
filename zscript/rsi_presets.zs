...
class RSI_Presets
{
	...
	// impact shape/tex/size/life, death shape/tex/size/life
	static void Look(int ish, int itx, double isz, int ilf,
	                 int dsh, int dtx, double dsz, int dlf)
	{
		I("rsi_impact_shape", ish);
		I("rsi_impact_tex",   itx);
		F("rsi_impact_size",  isz);
		I("rsi_impact_life",  ilf);
		I("rsi_death_shape",  dsh);
		I("rsi_death_tex",    dtx);
		F("rsi_death_size",   dsz);
		I("rsi_death_life",   dlf);
	}
	...
	static void Grid()
	{
		Look(4, 3,  88.0, 32,  10, 3, 210.0, 70);
	...
	{
		Look(-1, -1, 110.0, 35, -1, -1, 260.0, 80);
